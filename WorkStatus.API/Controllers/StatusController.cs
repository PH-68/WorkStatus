...
namespace WorkStatus.API.Controllers
{
    ...
    public class StatusController : Controller
    {
        ...
        private static readonly Dictionary<string, string> CitiesNames = new Dictionary<string, string>
        {
           {"基隆市","Keelung City"},
           {"臺北市","Taipei City"},
           {"新北市","New Taipei City"},
                ...
                for (int i = 0; i < doc.DocumentNode.SelectNodes("//*[@class='Table_Body']/tr").Count - 1; i++)
                {
                    ...
                    if (doc.DocumentNode.SelectSingleNode("//*[@class='Table_Body']/tr[" + (i + 1) + "]/td[2]/font[1]").OuterHtml.IndexOf("#FF0000") != -1) //放假
                    {
                        ...
                        CityStatus cityStatus = new CityStatus
                        {
                            ...
                            CityNameEn = CitiesNames[doc.DocumentNode.SelectNodes("//*[@class='Table_Body']/tr[" + (i + 1) + "]/td[1]/font")[0].InnerText],
                            IsCanceled = true,
                            Notes = Notes
                        };
                        citiesStatuses.Add(cityStatus);
